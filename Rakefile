...
namespace :migrate do
  ...
  schema = SQLAlchemy::Schema.new
  migration = SQLAlchemy::Migrations.new './database/sql/tables', schema

  desc('Create migration')
  task :create do
    name = ENV['NAME']

    raise StandardError, 'migration name is required' if name.nil?

    migration_name = name.gsub(' ', '_')

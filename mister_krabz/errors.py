class CouldntCreateWallet(Exception): pass # noqa
class CouldntGetWallets(Exception): pass # noqa
class CouldntUpdateWallets(Exception): pass # noqa
class WalletNotFound(Exception): pass # noqa


class CouldntCreateCategory(Exception): pass # noqa
class CouldntGetCategories(Exception): pass # noqa
class CategoryNotFound(Exception): pass # noqa
class ParentCategoryNotFound(Exception): pass # noqa


class CouldNotAddCategory(Exception): pass # noqa
